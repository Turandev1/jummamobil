...
export default function Sifreniunutdum() {
  ...
  useEffect(() => {
    const loadStep = async () => {
      try {
        const savedStep = await AsyncStorage.getItem("LASTSTEP");
  ...
  useEffect(() => {
    if (step !== null) {
      AsyncStorage.setItem("LASTSTEP", step.toString());
    }
  }, [step]);
    ...
    const subscription = AppState.addEventListener(
      ...
      async (nextAppState) => {
        if (appState.current.match(/active/) && nextAppState === "background") {
          if (step !== null) {
            await AsyncStorage.setItem("LASTSCREEN", "sifreniunutdum");
            await AsyncStorage.setItem("LASTSTEP", step.toString());
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      await AsyncStorage.removeItem("LASTSTEP");
  ...
  return (
    <View style={{ flex: 1, marginTop: insets.top + 15 }}>
      ...
      {/* Step 3: Yeni şifre */}
      {step === 3 && (
        <View style={{ paddingTop: insets.top,flex,alignItems:'center' }}>
